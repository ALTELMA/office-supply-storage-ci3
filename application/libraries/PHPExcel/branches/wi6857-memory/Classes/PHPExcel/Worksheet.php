...
class PHPExcel_Worksheet implements PHPExcel_IComparable
{
	...
	*/
    ...
    public function getStyle($pCellCoordinate = 'A1')
    {
		...
		$this->_selectedCell = $pCellCoordinate;
    ...
    public function duplicateStyleArray($pStyles = null, $pRange = '')
    {
    	if (is_array($pStyles)) {
			...
			$oldXfIndexes = array();
	   		for ($col = $rangeStart[0]; $col <= $rangeEnd[0]; ++$col) {
	   			for ($row = $rangeStart[1]; $row <= $rangeEnd[1]; ++$row) {
					$oldXfIndexes[$this->getCell(PHPExcel_Cell::stringFromColumnIndex($col) . $row)->getXfIndex()] = true;
	   			}
	   		}
			...
			foreach ($oldXfIndexes as $oldXfIndex => $dummy) {
				...
				if ($existingStyle = $this->_parent->getCellXfByHashCode($newStyle->getHashCode())) {
					// there is already such cell Xf in our collection
			...
			// Loop through cells again and update the XF index
	   		for ($col = $rangeStart[0]; $col <= $rangeEnd[0]; ++$col) {
	   			for ($row = $rangeStart[1]; $row <= $rangeEnd[1]; ++$row) {
					$oldXfIndex = $this->getCell(PHPExcel_Cell::stringFromColumnIndex($col) . $row)->getXfIndex();
					$this->getCell(PHPExcel_Cell::stringFromColumnIndex($col) . $row)->setXfIndex($newXfIndexes[$oldXfIndex]);
	   			}
	   		}

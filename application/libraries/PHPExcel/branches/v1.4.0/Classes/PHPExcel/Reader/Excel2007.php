										...
										if (isset($c->is->t)) {
											...
											foreach ($c->is->r as $run) {
												...
												if (isset($run->rPr->color["rgb"])) {
													$objText->getFont()->setColor( new PHPExcel_Style_Color( $run->rPr->color["rgb"] ) );
												}
						    	...
						    	$range = strtoupper($dataValidation["sqref"]);
						   		...
						   		for ($col = $rangeStart[0]; $col <= $rangeEnd[0]; $col++) {
						   			for ($row = $rangeStart[1]; $row <= $rangeEnd[1]; $row++) {
										$docValidation = $docSheet->getCell(PHPExcel_Cell::stringFromColumnIndex($col) . $row)->getDataValidation();
